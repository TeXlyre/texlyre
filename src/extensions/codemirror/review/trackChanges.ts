...
function findContainingChunk(
	...
	while (low < high) {
		...
		if (chunks[mid].openStart <= from) low = mid + 1;
		else high = mid;
	}
	...
	if (candidate) {
		for (const index of [...candidate.ancestorIndexes, candidateIndex]) {
			const chunk = chunks[index];
			if (chunk.openEnd <= from && to <= chunk.closeStart) return chunk;
		}
	}

	return chunks.find(
		(chunk) => chunk.openEnd <= from && to <= chunk.closeStart,
	);
}
...
function trackChange(
	...
	const containing = findContainingChunk(chunks, change.from, change.to);

	if (containing) {
		const currentLength = containing.closeStart - containing.openEnd;
		const nextLength =
			currentLength - (change.to - change.from) + change.insert.length;

		if (
			!containing.hasNestedComments &&
			nextLength !== containing.originalText.length
		) {
			return null;
		}
		...
		const restored = containing.hasNestedComments
			? visibleText(nextBody)
			: nextBody;

		if (restored !== containing.originalText) return null;
		return {
			change: {
...
export function createTrackChangesFilter(deps: TrackChangesDeps): Extension {
	return EditorState.transactionFilter.of((tr) => {
		...
		for (const raw of original) {
			...
			const normalized =
				alreadyNormalized ||
				!isInsideAnnotationTag(tr.startState, raw.from, raw.to)
					? null
