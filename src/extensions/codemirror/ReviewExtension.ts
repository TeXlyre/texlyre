...
export const reviewChunks = createDerivedTagRangeField<ReviewChunk>((doc) => {
	const reviews = reviewService.parseReviews(doc);
	const stack: number[] = [];

	return reviews.map((review, index) => {
		while (
			stack.length &&
			reviews[stack[stack.length - 1]].closeTagEnd <= review.openTagStart
		) {
			stack.pop();
		}

		const ancestorIndexes = stack.filter(
			(parentIndex) =>
				reviews[parentIndex].openTagEnd <= review.openTagStart &&
				reviews[parentIndex].closeTagStart >= review.closeTagEnd,
		);
		const chunk: ReviewChunk = {
			...
			hasNestedComments: containsAnnotationMarker(
				review.currentText,
				'comment',
			),
			ancestorIndexes,
...
export const reviewState = createReviewDecorationField(reviewChunks);
