		...
		update(value, tr) {
			if (!tr.docChanged) {
				return tr.startState.field(field) === tr.state.field(field)
					? value
					: compute(tr.state.field(field), tr.state.doc);
			}

			const ranges = tr.startState.field(field);
			const syntaxRanges =
				tr.startState.field(annotationIndex, false) ?? ranges;
			if (
				tagSyntaxUntouched(tr, ranges, syntaxRanges) &&
				(!bodySensitive || tagBodiesUntouched(tr, ranges))
			) {
