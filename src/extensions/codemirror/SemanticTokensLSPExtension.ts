...
function resolveClass(
    ...
    const extra: Tag[] = [];
    for (let index = 0; index < legend.tokenModifiers.length && index < 31; index++) {
...
function buildDecorations(
    ...
    for (let index = 0; index + 4 < data.length; index += 5) {
        ...
        const className = resolveClass(view.state, legend, data[index + 3], data[index + 4]);
            ...
            class {
                ...
                private async requestTokens() {
                    ...
                    try {
                        ...
                        if (generation !== this.generation || !this.view.dom.isConnected) return;
                        if (!Array.isArray(result?.data)) return;

                        this.answered = true;
                        this.decorations = buildDecorations(this.view, result.data, target.legend);
                        this.view.dispatch({});
                    } catch { }
                }
            },
